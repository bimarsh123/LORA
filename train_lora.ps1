...
# 参数化
param(
    [string]$pretrained_model = "", # base model path | 底模路径
    [string]$train_data_dir = "", # train dataset path | 训练数据集路径
    [string]$output_name = "", # output model name | 模型保存名称
    [int]$network_dim = 32, # network dim | 常用 4~128，不是越大越好
    [int]$network_alpha = 32, # network alpha | 常用与 network_dim 相同的值或者采用较小的值，如 network_dim的一半 防止下溢。默认值为 1，使用较小的 alpha 需要提升学习率。
    
    [string]$resolution = "512,512", # image resolution w,h. 图片分辨率，宽,高。支持非正方形，但必须是 64 倍数。
    [int]$batch_size = 1, # batch size
    [int]$max_train_epoches = 20, # max train epoches | 最大训练 epoch
    [int]$save_every_n_epochs = 2 # save every n epochs | 每 N 个 epoch
)
...
$outputDir = "./output" # output directory | 输出文件夹
$save_model_as = "safetensors" # model save ext | 模型保存格式 ckpt, pt, safetensors
$destinationDir = "D:\code\stable-diffusion-webui\models\Lora" # 复制模型路径
...
# 优化器设置
$optimizer_type = "AdamW" # Optimizer type | 优化器类型 默认为 AdamW8bit，可选：AdamW AdamW8bit Lion Lion8bit (需自行安装 bitsandbytes >= 0.38) SGDNesterov SGDNesterov8bit DAdaptation AdaFactor prodigy (需自行安装 prodigyopt)
...
if (Test-Path $outputDir) {
    Write-Output "Model folder exists"
} else {
    Write-Output "Model folder does not exist"
    exit
}
...
# 构建完成文件路径
$fileToCopy = Join-Path -Path $outputDir -ChildPath "$output_name.safetensors"
...
# 复制文件
Copy-Item -Path $fileToCopy -Destination $destinationDir -Force
